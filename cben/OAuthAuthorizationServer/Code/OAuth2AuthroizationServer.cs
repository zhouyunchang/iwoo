    ...
    internal class OAuth2AuthorizationServer : IAuthorizationServerHost
    {
        ...
        private bool IsAuthorizationValid(HashSet<string> requestedScopes, string clientIdentifier, DateTime issuedUtc, string username)
        {
            ...
            var grantedScopeStrings = from auth in MvcApplication.DataContext.ClientAuthorizations
                                      join client in MvcApplication.DataContext.Clients on auth.ClientId equals client.ClientId
                                      join user in MvcApplication.DataContext.Users on auth.UserId equals user.UserId
                                      where client.ClientIdentifier == clientIdentifier &&
                                          auth.CreatedOnUtc <= issuedUtc &&
                                          (!auth.ExpirationDateUtc.HasValue || auth.ExpirationDateUtc.Value >= DateTime.UtcNow) &&
                                          user.OpenIDClaimedIdentifier == username
                                      select auth.Scope;
