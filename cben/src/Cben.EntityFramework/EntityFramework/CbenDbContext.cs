...
namespace Cben.EntityFramework
{
    ...
    /// </summary>
    public abstract class CbenDbContext : DbContext, ITransientDependency, IShouldInitialize
    {
        /// <summary>
        ...
        /// <summary>
        /// Constructor.
        /// Uses <see cref="ICbenStartupConfiguration.DefaultNameOrConnectionString"/> as connection string.
        /// </summary>
        protected CbenDbContext()
        {
            InitializeDbContext();
        }
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(string nameOrConnectionString)
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(DbCompiledModel model)
            : base(model)
        {
            InitializeDbContext();
        }
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(DbConnection existingConnection, bool contextOwnsConnection)
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(string nameOrConnectionString, DbCompiledModel model)
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(ObjectContext objectContext, bool dbContextOwnsObjectContext)
        ...
        /// Constructor.
        /// </summary>
        protected CbenDbContext(DbConnection existingConnection, DbCompiledModel model, bool contextOwnsConnection)
        ...
        public virtual void Initialize()
        {
            ...
            this.SetFilterScopedParameterValue(CbenDataFilters.MustHaveTenant, CbenDataFilters.Parameters.TenantId, CbenSession.TenantId ?? 0);
            this.SetFilterScopedParameterValue(CbenDataFilters.MayHaveTenant, CbenDataFilters.Parameters.TenantId, CbenSession.TenantId);
        }

        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            base.OnModelCreating(modelBuilder);
            modelBuilder.Filter(CbenDataFilters.SoftDelete, (ISoftDelete d) => d.IsDeleted, false);
            modelBuilder.Filter(CbenDataFilters.MustHaveTenant, (IMustHaveTenant t, int tenantId) => t.TenantId == tenantId || (int?)t.TenantId == null, 0); //While "(int?)t.TenantId == null" seems wrong, it's needed. See https://github.com/jcachat/EntityFramework.DynamicFilters/issues/62#issuecomment-208198058
            modelBuilder.Filter(CbenDataFilters.MayHaveTenant, (IMayHaveTenant t, int? tenantId) => t.TenantId == tenantId, 0);
        }
        ...
        public override int SaveChanges()
        {
            try
            {
                var changedEntities = ApplyCbenConcepts();
        ...
        public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken)
        {
            try
            {
                var changeReport = ApplyCbenConcepts();
        ...
        protected virtual EntityChangeReport ApplyCbenConcepts()
        {
        ...
        protected virtual void CheckAndSetMustHaveTenantIdProperty(object entityAsObj)
        {
            ...
            {
                throw new CbenException("Can not set TenantId to 0 for IMustHaveTenant entities!");
            }
        }
        ...
        protected virtual void CheckAndSetMayHaveTenantIdProperty(object entityAsObj)
        {
            ...
            if (!this.IsFilterEnabled(CbenDataFilters.MayHaveTenant))
            {
                return;
            }
            ...
            if (userId.HasValue && entityAsObj is ICreationAudited)
            {
                ...
                if (entity.CreatorUserId == null)
                {
                    if (entity is IMayHaveTenant || entity is IMustHaveTenant)
                    {
                        ...
                        if ((entity is IMayHaveTenant && entity.As<IMayHaveTenant>().TenantId == CbenSession.TenantId) ||
                            (entity is IMustHaveTenant && entity.As<IMustHaveTenant>().TenantId == CbenSession.TenantId))
                        {
        ...
        protected virtual long? GetAuditUserId()
        {
            if (CbenSession.UserId.HasValue &&
                ...
                CurrentUnitOfWorkProvider.Current.GetTenantId() == CbenSession.TenantId)
            {
                return CbenSession.UserId;
            }
