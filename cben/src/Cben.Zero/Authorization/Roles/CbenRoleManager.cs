...
namespace Cben.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class CbenRoleManager<TRole, TUser>
        ...
        where TUser : CbenUser<TUser>
    {
        ...
        private IRolePermissionStore<TRole, TUser> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new CbenException("Store is not IRolePermissionStore");
                }
        ...
        protected CbenRoleStore<TRole, TUser> CbenStore { get; private set; }
        ...
        /// Constructor.
        /// </summary>
        protected CbenRoleManager(
            CbenRoleStore<TRole, TUser> store,
        ...
        public async override Task<IdentityResult> DeleteAsync(TRole role)
        {
            if (role.IsStatic)
            {
                return CbenIdentityResult.Failed(string.Format(L("CanNotDeleteStaticRole"), role.Name));
            }
        ...
        /// <exception cref="CbenException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId);
            if (role == null)
            {
                throw new CbenException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="CbenException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new CbenException("There is no role with name: " + roleName);
            }
        ...
        public virtual async Task<IdentityResult> CheckDuplicateRoleNameAsync(int? expectedRoleId, string name, string displayName)
        {
            ...
            {
                return CbenIdentityResult.Failed(string.Format(L("RoleNameIsAlreadyTaken"), name));
            }

            role = await FindByDisplayNameAsync(displayName);
            if (role != null && role.Id != expectedRoleId)
            {
                return CbenIdentityResult.Failed(string.Format(L("RoleDisplayNameIsAlreadyTaken"), displayName));
            }
        ...
        {
            return CbenStore.FindByDisplayNameAsync(displayName);
        }
