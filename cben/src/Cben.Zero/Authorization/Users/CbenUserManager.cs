...
namespace Cben.Authorization.Users
{
    ...
    /// </summary>
    public abstract class CbenUserManager<TRole, TUser>
        ...
        where TUser : CbenUser<TUser>
    {
        protected IUserPermissionStore<TUser> UserPermissionStore
        {
            get
            {
                ...
                {
                    throw new CbenException("Store is not IUserPermissionStore");
                }
        ...
        public CbenUserStore<TRole, TUser> CbenStore { get; }
        ...
        protected CbenUserManager(
            CbenUserStore<TRole, TUser> userStore,
        ...
        public virtual async Task<bool> IsGrantedAsync(long userId, Permission permission)
        {
            ...
            if (!permission.MultiTenancySides.HasFlag(CbenSession.MultiTenancySide))
            {
                return false;
            }

            //Check for depended features
            if (permission.FeatureDependency != null && CbenSession.MultiTenancySide == MultiTenancySides.Tenant)
            {
        ...
        {
            return await CbenStore.FindByNameOrEmailAsync(userNameOrEmailAddress);
        }

        public virtual Task<List<TUser>> FindAllAsync(UserLoginInfo login)
        {
            return CbenStore.FindAllAsync(login);
        }
        ...
        /// <exception cref="CbenException">Throws exception if no user found with given id</exception>
        public virtual async Task<TUser> GetUserByIdAsync(long userId)
        {
            var user = await FindByIdAsync(userId);
            if (user == null)
            {
                throw new CbenException("There is no user with id: " + userId);
            }
        ...
        public async override Task<ClaimsIdentity> CreateIdentityAsync(TUser user, string authenticationType)
        {
            ...
            {
                identity.AddClaim(new Claim(CbenClaimTypes.TenantId, user.TenantId.Value.ToString(CultureInfo.InvariantCulture)));
            }
        ...
        public async override Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (user.UserName != CbenUser<TUser>.AdminUserName)
            {
                if ((await GetOldUserNameAsync(user.Id)) == CbenUser<TUser>.AdminUserName)
                {
                    return CbenIdentityResult.Failed(string.Format(L("CanNotRenameAdminUser"), CbenUser<TUser>.AdminUserName));
                }
            }
        ...
        public async override Task<IdentityResult> DeleteAsync(TUser user)
        {
            if (user.UserName == CbenUser<TUser>.AdminUserName)
            {
                return CbenIdentityResult.Failed(string.Format(L("CanNotDeleteAdminUser"), CbenUser<TUser>.AdminUserName));
            }
        ...
        public virtual async Task<IdentityResult> ChangePasswordAsync(TUser user, string newPassword)
        {
            ...
            await CbenStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(newPassword));
        ...
        public virtual async Task<IdentityResult> CheckDuplicateUsernameOrEmailAddressAsync(long? expectedUserId, string userName, string emailAddress)
        {
            ...
            {
                return CbenIdentityResult.Failed(string.Format(L("Identity.DuplicateName"), userName));
            }

            user = (await FindByEmailAsync(emailAddress));
            if (user != null && user.Id != expectedUserId)
            {
                return CbenIdentityResult.Failed(string.Format(L("Identity.DuplicateEmail"), emailAddress));
            }
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new CbenException(string.Format("Can not set more than {0} organization unit for a user!", maxCount));
            }
        }
        ...
        public virtual Task<List<TUser>> GetUsersInOrganizationUnit(OrganizationUnit organizationUnit, bool includeChildren = false)
        {
            ...
            {
                var query = from uou in _userOrganizationUnitRepository.GetAll()
                            join user in CbenStore.Users on uou.UserId equals user.Id
        ...
        public virtual void RegisterTwoFactorProviders(int? tenantId)
        {
            ...
            if (!IsTrue(CbenZeroSettingNames.UserManagement.TwoFactorLogin.IsEnabled, tenantId))
            {
                return;
            }

            if (EmailService != null &&
                IsTrue(CbenZeroSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, tenantId))
            {
            ...
            if (SmsService != null &&
                IsTrue(CbenZeroSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, tenantId))
            {
                RegisterTwoFactorProvider(
                    L("Sms"),
        ...
        {
            UserLockoutEnabledByDefault = IsTrue(CbenZeroSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            DefaultAccountLockoutTimeSpan = TimeSpan.FromSeconds(GetSettingValue<int>(CbenZeroSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            MaxFailedAccessAttemptsBeforeLockout = GetSettingValue<int>(CbenZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);
        }
        ...
        {
            return CbenStore.GetUserNameFromDatabaseAsync(userId);
        }
