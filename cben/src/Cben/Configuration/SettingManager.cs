...
namespace Cben.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, CbenSession.TenantId, CbenSession.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&
                        ((setting.Scopes.HasFlag(SettingScopes.Tenant) && CbenSession.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && CbenSession.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && CbenSession.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(CbenSession.TenantId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    ...
                    if (!setting.IsInherited &&
                        (setting.Scopes.HasFlag(SettingScopes.User) && CbenSession.UserId.HasValue))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.User) && CbenSession.UserId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForUserAsync(CbenSession.ToUserIdentifier()))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
        ...
        {
            return GetAllSettingValuesForUserAsync(new UserIdentifier(CbenSession.TenantId, userId));
        }
        ...
        {
            return ChangeSettingForUserAsync(new UserIdentifier(CbenSession.TenantId, userId), name, value);
        }
